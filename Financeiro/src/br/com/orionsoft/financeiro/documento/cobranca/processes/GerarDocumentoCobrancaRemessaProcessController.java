 ...
 * 
 * @author Lucio 27/01/2011
 * @version 20110127
 * 
 * @spring.bean id="GerarDocumentoCobrancaRemessaProcessController" init-method="registerController"
 * @spring.property name="processManager" ref="ProcessManager"
 */
public class GerarDocumentoCobrancaRemessaProcessController extends RunnableProcessControllerBasic implements IRunnableEntityProcessController {
	...
	public Class<? extends IProcess> getProcessClass() {return GerarDocumentoCobrancaRemessaProcess.class;}
		...
		try{
			...
			if(entity.getInfo().getType() == DocumentoCobranca.class){
				...
				if(entity.getProperty(DocumentoCobranca.CONVENIO_COBRANCA).getValue().isValueNull()){
					this.setMessage(new BusinessMessage(GerarDocumentoCobrancaRemessaProcessController.class, "DOCUMENTO_SEM_CONVENIO"));
					return false;
				}
			} else	
			/* Verifica se a entidade é compatível */
			if(entity.getInfo().getType() == DocumentoCobrancaCategoria.class){
				...
				if(entity.getProperty(DocumentoCobrancaCategoria.CONVENIO_COBRANCA).getValue().isValueNull()){
					this.setMessage(new BusinessMessage(GerarDocumentoCobrancaRemessaProcessController.class, "DOCUMENTO_CATEGORIA_SEM_CONVENIO"));
					return false;
				}
			}	
