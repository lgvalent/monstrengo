...
<a4j:region id="inserirPagamento" selfRendered="true">
	<a4j:status>
		<f:facet name="start">
			<f:verbatim>
...
<h:form id="form" >
	...
	<h:panelGrid columns="3" cellpadding="3">
		<h:outputLabel value="Selecione um documento de pagamento já disponível nos lançamentos:"/>
		<h:selectOneMenu id="documentoPagamento" value="#{quitarLancamentoBean.process.documentoPagamentoId}">
			<f:selectItems value="#{quitarLancamentoBean.process.listDocumentoPagamentoDisponiveis}"/>
			<a4j:support event="onchange" action="#{quitarLancamentoBean.doPrepararDocumentoPagamento}" reRender="form" ajaxSingle="false"/>
		</h:selectOneMenu>
		<h:message styleClass="errorMessage" for="documentoPagamento"/>
	</h:panelGrid>
		
	<h:panelGroup id="categoria">
		<h:panelGrid columns="3" cellpadding="3" rendered="#{quitarLancamentoBean.process.documentoPagamentoId==-1}">
			<h:outputLabel value="Selecione o tipo de documento de pagamento que será vinculado a este lancamento:"/>
			<h:selectOneMenu id="documentoPagamentoCategoria" value="#{quitarLancamentoBean.process.documentoPagamentoCategoria.id}">
				<f:selectItems value="#{quitarLancamentoBean.process.listDocumentoPagamentoCategoria}"/>
				<a4j:support event="onchange" action="#{quitarLancamentoBean.doPrepararDocumentoPagamento}" reRender="propriedades" ajaxSingle="false"/>
			</h:selectOneMenu>
			<h:message styleClass="errorMessage" for="documentoPagamento"/>
		</h:panelGrid>
	</h:panelGroup>		
	<h:panelGroup id="propriedades">
		<h:panelGrid rendered="#{!quitarLancamentoBean.temPropriedadesEditaveis}">
			<h:outputLabel value="Nenhuma informação adicional para este documento de pagamento é necessária." styleClass="infoMessage"/>
		</h:panelGrid>

		<h:panelGrid rendered="#{quitarLancamentoBean.temPropriedadesEditaveis}">
			<h:outputLabel value="Preencha as informações adicionais sobre o documento de pagamento " styleClass="infoMessage"/>
			<h:dataTable border="0" 
					 value="#{quitarLancamentoBean.propriedadesEditaveis}" 
		             var='item'
		             rendered="#{quitarLancamentoBean.temPropriedadesEditaveis}"					
		             width="100%"
					...
					<h:column id="values" rendered="#{item.info.visible && !item.info.calculated && !item.info.readOnly}">
						...
						<h:panelGroup rendered="#{item.info.editShowList && item.info.entity && !item.info.collection}">
							...
							<h:outputLink value="javascript:linkCreatePopup('#{item.info.type.name}')">
			   					<h:graphicImage value="../../public/basic/img/create.png" title="Cria um novo registro. Será necessário clicar em validar para atualizar a lista com o novo item" style="border:0"/>
							</h:outputLink>
							<h:outputLink value="javascript:document.forms[0].submit()">
			   					<h:graphicImage value="../../public/basic/img/reload.png" title="Recarrega a página e a lista de seleção" style="border:0"/>
							</h:outputLink>
						</h:panelGroup>
	...
	<h:panelGrid columns="3">
		...
		<h:commandButton value="Avançar >>" action="#{quitarLancamentoBean.actionPasso3}"/>
