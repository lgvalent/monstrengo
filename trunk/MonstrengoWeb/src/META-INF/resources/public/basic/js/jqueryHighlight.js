...
/* Lucio 
 * Functions to highlight a single word or a list of words separated with black space
 * 20130613
 */
function highlightWords(phrase, container) {
	...
	for (i in words) {
		jQuery('#'+container).highlight(words[i], false);
	}
}

function highlightWord(word, container) {
	jQuery('#'+container).removeHighlight();
	jQuery('#'+container).highlight(word, false);
}