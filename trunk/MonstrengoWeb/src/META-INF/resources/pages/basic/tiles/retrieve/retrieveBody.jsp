		...
		<h:panelGrid columns="2" width="100%" style="color:#{retrieveBean.currentEntity.info.colorName}">
			<h:panelGrid styleClass="noprint" columns="7" style="vertical-align: top">
			    	<h:outputLink value="javascript:linkCheckAuditCrud('#{retrieveBean.currentEntity.info.type.name}', #{retrieveBean.currentEntity.id})" rendered="#{menuBean.crudMap.AuditCrudRegister.canRetrieve}">
						<h:graphicImage value="../basic/img/checkAuditCrud.png" title="Verifica quem criou e alterou este registro" style="border:0" />
				    </h:outputLink>
					...
					<h:graphicImage value="../basic/img/update_d.png" title="Você não possui direitos para alterar os dados do registro" style="border:0" rendered="#{!retrieveBean.canUpdate}"/>
					...
					<h:graphicImage value="../basic/img/delete_d.png" title="Você não possui direitos para apagar o registro" style="border:0" rendered="#{!retrieveBean.canDelete}"/>
				
		  			<h:commandLink action="#{createBean.actionCreate}" rendered="#{retrieveBean.canCreate}">
						<h:graphicImage value="../basic/img/create.png" title="Cria um novo registro" style="border:0"/>
				     	<f:param name="entityType" value="#{retrieveBean.currentEntity.info.type.name}"/>
				     	<f:param name="entityId" value="#{retrieveBean.entityParam.id}"/>
					</h:commandLink>
					<h:graphicImage value="../basic/img/create_d.png" title="Você não possui direitos para criar um novo registro" style="border:0" rendered="#{!retrieveBean.canCreate}"/>
					...
					<h:outputLink value="javascript: closeSelectOne('#{retrieveBean.currentParams.selectOneDest}','#{retrieveBean.selectPropertyValue}',window)" rendered="#{retrieveBean.selectOneActive}">
		  			...
		  			<h:outputLink value="javascript:window.close()" rendered="#{!retrieveBean.selectOneActive}">
						<h:graphicImage value="../basic/img/close.png" title="Fecha esta visualização" style="border:0"/>
					</h:outputLink>
			</h:panelGrid>
							...
							<h:column>
								<f:facet name="footer">
									<h:outputLink value="javascript:linkQueryParent('#{retrieveBean.currentEntity.info.type.name}', '#{retrieveBean.currentEntity.id}', '#{item.info.name}')">
										<h:graphicImage value="../basic/img/query.png" title="Pesquisar na lista de #{item.info.label}" style="border:0" />
...
<%-- POR ACTION --%>
			    	<h:outputLink value="javascript:linkCheckAuditCrud('#{retrieveBean.currentEntity.info.type.name}', #{retrieveBean.currentEntity.id})" rendered="#{menuBean.crudMap.AuditCrudRegister.canRetrieve}">
						<h:graphicImage value="../basic/img/checkAuditCrud_b.png" title="Verifica quem criou e alterou este registro" style="border:0" />
				    </h:outputLink>

	  				<h:commandLink action="#{updateBean.actionEdit}" rendered="#{retrieveBean.canUpdate}">
						<h:graphicImage value="../basic/img/update_b.png" title="Altera os dados do registro" style="border:0"/>
				     	<f:param name="entityType" value="#{retrieveBean.currentEntity.info.type.name}"/>
				     	<f:param name="entityId"   value="#{retrieveBean.entityParam.id}"/>
					</h:commandLink>
					<h:graphicImage value="../basic/img/update_b_d.png" title="Você não possui direitos para alterar os dados do registro" style="border:0" rendered="#{!retrieveBean.canUpdate}"/>
					
		  			<h:commandLink action="#{deleteBean.actionDelete}" rendered="#{retrieveBean.canDelete}">
						<h:graphicImage value="../basic/img/delete_b.png" title="Apaga o registro" style="border:0"/>
				     	<f:param name="entityType" value="#{retrieveBean.currentEntity.info.type.name}"/>
				     	<f:param name="entityId"   value="#{retrieveBean.entityParam.id}"/>
					</h:commandLink>
					<h:graphicImage value="../basic/img/delete_b_d.png" title="Você não possui direitos para apagar o registro" style="border:0" rendered="#{!retrieveBean.canDelete}"/>
				
		  			<h:commandLink action="#{createBean.actionCreate}" rendered="#{retrieveBean.canCreate}">
						<h:graphicImage value="../basic/img/create_b.png" title="Cria um novo registro" style="border:0"/>
				     	<f:param name="entityType" value="#{retrieveBean.currentEntity.info.type.name}"/>
				     	<f:param name="entityId" value="#{retrieveBean.entityParam.id}"/>
					</h:commandLink>
					<h:graphicImage value="../basic/img/create_b_d.png" title="Você não possui direitos para criar um novo registro" style="border:0" rendered="#{!retrieveBean.canCreate}"/>
				
					<h:outputLink value="javascript: closeSelectOne('#{retrieveBean.currentParams.selectOneDest}','#{retrieveBean.selectPropertyValue}',window)" rendered="#{retrieveBean.selectOneActive}">
						<h:graphicImage style="border: none;" value="../basic/img/query_select_one_b.png" alt="Clique para selecionar este registro e voltar para a tela anterior"/>
					</h:outputLink>

		  			<h:outputLink value="javascript:window.close()" rendered="#{!retrieveBean.selectOneActive}">
						<h:graphicImage value="../basic/img/close_b.png" title="Fecha esta visualização" style="border:0"/>
					</h:outputLink>
			</h:panelGrid>
		</h:panelGrid>
	    <%-- f:verbatim>
 	...
 	<h:form rendered="#{menuBean.crudMap.AddressLabel.canCreate && labelEntityBean.hasModelsLabelEntity(retrieveBean.currentEntity)}" styleClass="noprint">
		...
		<h:panelGrid columns="2">
			<h:graphicImage value="../basic/img/label.png" style="border:0"/>
			<h:panelGroup>
 	...
 	<h:form target="_blank" styleClass="noprint">
		...
		<h:panelGrid columns="2" rendered="#{menuBean.processMap.CompileDocumentProcess && documentEntityBean.hasModelsDocumentEntity(retrieveBean.currentEntity)}">
			<h:graphicImage value="../basic/img/document.png" style="border:0"/>
			<h:panelGroup>
		...
		<h:dataTable
			...
			<h:column>
				<h:panelGroup rendered="#{runnableView.runnableProcessEntry.disabled}">
					<h:graphicImage value="../basic/img/run_task_d.png" style="border:0"/>
