...
<h:form id="form" >
	<h:panelGrid columns="2">
		<h:graphicImage value="./img/documentoPagamento.png" width="96" height="96"/>
		<h:outputLabel value="O documento de pagamento <b>previsto</b> define o documento que será utilizado durante a quitação deste lançamento, seja ele de entrada ou de saída.<br>Com isto, é possível emitir os documentos de pagamento mesmo antes da quitação efetiva do lançamento, pois este documento de pagamento será utilizado para a quitação no momento em que o operador registrar esta ação." styleClass="description"/>
	</h:panelGrid>
	<h:panelGrid columns="3" cellpadding="3">
		<h:outputLabel value="Selecione o tipo de documento de pagamento que será vinculado a este lançamento:"/>
		<h:selectOneMenu id="documentoPagamento" value="#{inserirLancamentoBean.process.documentoPagamentoCategoria.id}" >
			<f:selectItems value="#{inserirLancamentoBean.process.listDocumentoPagamentoCategoria}"/>
			<a4j:support event="onchange" action="#{inserirLancamentoBean.doCriarDocumentoPagamento}" reRender="propriedades" ajaxSingle="true"/>
		</h:selectOneMenu>
		...
		<h:dataTable border="0" 
					...
					<h:column id="values" rendered="#{item.info.visible && !item.info.calculated && !item.info.readOnly}">
						...
						<h:panelGroup rendered="#{item.info.editShowList && item.info.entity && !item.info.collection}">
							<h:selectOneMenu id="selectSubClassInput" value="#{item.value.id}" > <%-- required="#{item.info.required}" Lucio 20090119: Se requerido, ao teocar a categoria fica dando uma mensagem de 'requerido' nesta propriedade --%> 
        	               		<f:selectItems value="#{item.valuesList}"/>
            	        	</h:selectOneMenu>
	...
	<h:panelGrid columns="2">
		...
		<h:commandButton value="Avançar >>" action="#{inserirLancamentoBean.actionDocumentoPagamentoCategoria}"/>
	</h:panelGrid>
</h:form>
